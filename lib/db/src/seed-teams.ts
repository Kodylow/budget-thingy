...
export const SEEDED_TEAM_LIMIT_TARGETS: SeededTeamLimitTarget[] = [
  ...
  { teamName: "Comcast Business Customer Solutions", workspaceId: "66ox9cntlf", groupId: "biqK255d", groupName: "AZ-Replit - Comcast Business Customer Solutions - Member" },
...
export const BASELINE_GROUP_TEAMS: { groupName: string; teamName: string }[] = [
  ...
  { groupName: "AZ-Replit - Comcast Business Consumer Solutions - Admin", teamName: "Comcast Business Customer Solutions" },
  { groupName: "AZ-Replit - Comcast Business Consumer Solutions - Member", teamName: "Comcast Business Customer Solutions" },
  { groupName: "AZ-Replit - Comcast Business Consumer Solutions - Viewer", teamName: "Comcast Business Customer Solutions" },
...
export const ORIGINAL_TEAM_BUDGETS: { teamName: string; amountUsd: number; isHidden?: boolean }[] = [
  ...
  { teamName: "Comcast Business Customer Solutions", amountUsd: 37473.54 },
...
export interface AnnualTeamBudgetBackfillResult {
  /**
   * This former allocation may contain operator-managed history. Seeding does
   * not have enough provenance to retire it safely, so callers can surface it
   * for an explicit operator decision.
   */
  legacyBudgetRequiresReview: boolean;
  writeStatements: 1;
  configurationRevisionAdvances: 1;
}
export async function applyAnnualTeamBudgetBackfill(): Promise<AnnualTeamBudgetBackfillResult> {
  return db.transaction(insertAnnualTeamBudgetDefaults);
}

function buildSeedFamilyMappings(): (typeof familyTeamMappingsTable.$inferInsert)[] {
  ...
  const mappings: (typeof familyTeamMappingsTable.$inferInsert)[] = [
    ...[...legacyFamilies].map(([familyKey, family]) => ({
  ...
  for (const target of SEEDED_TEAM_LIMIT_TARGETS) {
    ...
    mappings.push({
      ...
      familyKey,
      familyName,
      teamName: FAMILY_TEAM_OVERRIDES.get(familyKey) ?? target.teamName,
      isLegacy: false,
    });
  }
  return mappings;
...
export async function applyTeamMappingAndLimitTargetSeed(): Promise<void> {
  await db.transaction(insertTeamMappingAndLimitTargetDefaults);
}

/**
 * Installs all canonical defaults through a caller-owned executor. Bootstrap
 * passes a Drizzle instance bound to its migration transaction's pg client;
 * standalone callers should use seedDatabase(), which owns the transaction.
 */
export async function seedCanonicalRows(
  tx: CanonicalSeedExecutor,
): Promise<CanonicalSeedResult> {
  const annualBackfill = await insertAnnualTeamBudgetDefaults(tx);
  await insertTeamMappingAndLimitTargetDefaults(tx);
  return {
    legacyBudgetRequiresReview: annualBackfill.legacyBudgetRequiresReview,
    writeStatements: 4,
    configurationRevisionAdvances: 3,
  };
}
export async function seedDatabase(): Promise<CanonicalSeedResult> {
  console.log("Seeding team mappings, allocations, overrides, and limit targets...");
  const result = await db.transaction(seedCanonicalRows);
  if (result.legacyBudgetRequiresReview) {
    console.warn(
      'Preserved legacy "Growth Strategy & Operations" budget; review it for explicit retirement.',
    );
  }
...
export type CanonicalSeedExecutor = Pick<SeedTransaction, "insert" | "select">;

async function insertTeamMappingAndLimitTargetDefaults(
  tx: CanonicalSeedExecutor,
): Promise<void> {
  await tx
    .insert(familyTeamMappingsTable)
    .values(buildSeedFamilyMappings())
    .onConflictDoNothing();

  await tx
    .insert(teamLimitTargetsTable)
    .values(SEEDED_TEAM_LIMIT_TARGETS.map((target) => {
      const familyKey = normalizeSeedFamilyKey(
        familyNameFromGroupName(target.groupName),
      );
      return {
        ...target,
        teamName: FAMILY_TEAM_OVERRIDES.get(familyKey) ?? target.teamName,
...
async function insertAnnualTeamBudgetDefaults(
  tx: CanonicalSeedExecutor,
): Promise<AnnualTeamBudgetBackfillResult> {
  await tx
    .insert(teamBudgetsTable)
    .values(ORIGINAL_TEAM_BUDGETS.map((budget) => ({
      ...budget,
  ...
  return {
    legacyBudgetRequiresReview: Boolean(legacy),
    writeStatements: 1,
    configurationRevisionAdvances: 1,
  };
}
...
export interface CanonicalSeedResult {
  legacyBudgetRequiresReview: boolean;
  /** Four batched INSERT statements, irrespective of the number of defaults. */
  writeStatements: 4;
  /**
   * The committed statement-level triggers cover budgets, family mappings, and
   * exact targets. Workspace defaults intentionally have no revision trigger.
   */
  configurationRevisionAdvances: 3;
}
