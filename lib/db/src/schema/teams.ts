...
export const teamBudgetAllocationAuditsTable = pgTable(
  "team_budget_allocation_audits",
  {
    ...
    teamName: text("team_name").notNull(),
    field: text("field")
      .$type<"annualAllocationUsd" | "isHidden" | "monthlyAllocationAddition">()
      .notNull(),
    oldValue: jsonb("old_value").$type<number | boolean | { month: string; amountUsd: number }>().notNull(),
    newValue: jsonb("new_value").$type<number | boolean | { month: string; amountUsd: number }>().notNull(),
...
export const teamBudgetAdjustmentsTable = pgTable(
  "team_budget_adjustments",
  {
    ...
    sourceKind: text("source_kind").notNull().default("approved_credit"),
...
export const teamBudgetSyncStateTable = pgTable("team_budget_sync_state", {
  ...
  sourceBaseId: text("source_base_id"),
  sourceTableId: text("source_table_id"),
  sourceAvailable: boolean("source_available").notNull().default(false),
