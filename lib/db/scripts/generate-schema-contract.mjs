...
try {
  ...
  for (const migration of migrations.slice(1)) {
    await reset();
    // Compare each forward migration with the schema produced by every prior
    // migration. This is required to record removals and alterations of objects
    // introduced after the rebased baseline.
    for (const prerequisite of migrations.slice(1, migration.idx)) {
      await execute(prerequisite);
    }
    const before = await snapshot(client, "contract");
    await execute(migration);
    const after = await snapshot(client, "contract");
    canonicalShapes.set(migration.idx, after);
    deltas[migration.idx] = delta(before, after);
  }
  // The rebased 0000 clock can skip any remaining 0001–0004 entries.
  // Idempotent forward repairs may be no-ops on the full history but still
  // change these supported legacy paths. Record those postconditions too.
  for (let legacyPrefix = 1; legacyPrefix < 5; legacyPrefix++) {
    await reset();
    for (const migration of migrations.slice(1, legacyPrefix)) await execute(migration);
    for (const migration of migrations.slice(5)) {
      const before = await snapshot(client, "contract");
      await execute(migration);
      const changes = delta(before, await snapshot(client, "contract"));
      const canonical = canonicalShapes.get(migration.idx);
      const combined = deltas[migration.idx];
      for (const [category, entries] of Object.entries(changes.set)) {
        for (const [key, value] of Object.entries(entries)) {
          if (JSON.stringify(value) !== JSON.stringify(canonical[category][key])) {
            throw new Error(`Legacy repair differs from canonical schema: ${migration.tag} ${category}.${key}`);
          }
          (combined.set[category] ??= {})[key] = value;
        }
      }
      for (const [category, keys] of Object.entries(changes.remove)) {
        for (const key of keys) {
          if (key in canonical[category]) {
            throw new Error(`Legacy repair removes a canonical object: ${migration.tag} ${category}.${key}`);
          }
        }
        combined.remove[category] = [...new Set([...(combined.remove[category] ?? []), ...keys])];
      }
    }
  }
  const manifest = {
