...
describe("test migration safety", () => {
  ...
  it("keeps the configuration revision migration newer than its predecessors and covers every source table", () => {
    const migrationsRoot = new URL("../../../../lib/db/drizzle/", import.meta.url);
    const journal = JSON.parse(
      readFileSync(new URL("meta/_journal.json", migrationsRoot), "utf8"),
    ) as { entries: Array<{ idx: number; tag: string; when: number }> };
    const migration = journal.entries.find((entry) =>
      entry.tag === "0008_transactional_configuration_revision"
    );
    expect(migration).toBeDefined();
    expect(migration!.when).toBeGreaterThan(
      Math.max(...journal.entries
        .filter((entry) => entry.idx < migration!.idx)
        .map((entry) => entry.when)),
    );

    const sql = readFileSync(
      new URL(`${migration!.tag}.sql`, migrationsRoot),
      "utf8",
    );
    for (const table of [
      "group_budgets",
      ...
      expect(sql).toContain(`"${table}_configuration_dml"`);
      expect(sql).toContain(`"${table}_configuration_truncate"`);
    }
  ...
  it.each([
    ...
    await expect(applyTestMigrations({ query }, [{
      ...
      sql: `
        ...
        ${dangerousSql}
      `,
    }])).rejects.toThrow("unsafe schema-qualified SQL");
