...
import {
  ...
  ListWorkspaceUsageLimitAuditsQueryParams,
...
router.get(
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      const workspaceUsageComplete =
        usage.workspaceStatus === "complete" || usage.workspaceStatus === "stale";
...
router.get(
  ...
  requireUserLimitWorkspace,
  async (req, res): Promise<void> => {
    const parsed = ListWorkspaceUsageLimitAuditsQueryParams.safeParse(req.query);
    if (
      !parsed.success ||
      (
      ...
      res.status(400).json({ error: "Invalid usage-limit audit cursor" });
      return;
    }
    const workspaceId = String(req.params["workspaceId"]);
    const dir = await getDirectory();
    ...
    const workspacePredicate = eq(usageLimitAuditsTable.workspaceId, workspaceId);
    const rows = await db
      .select()
      .from(usageLimitAuditsTable)
      .where(parsed.data.beforeId === undefined
        ? workspacePredicate
        : and(
            workspacePredicate,
            lt(usageLimitAuditsTable.id, parsed.data.beforeId),
          ))
      .orderBy(desc(usageLimitAuditsTable.id))
      .limit(200);
