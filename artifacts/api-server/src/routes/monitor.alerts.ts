...
router.get("/alerts", async (req, res): Promise<void> => {
  ...
  if (!parsed.success) {
    res.status(400).json({ error: "Invalid alert history pagination" });
    return;
  }
  const limit = parsed.data.limit ?? 100;
  const beforeId = parsed.data.beforeId;
  ...
  try {
    ...
    // Current usage is optional enrichment of an immutable send-time history.
    // Its outage must not be confused with an unavailable authorization scope.
    try {
    ...
    for (const [teamName, spend] of byTeam) {
      ...
      currentByEntity.clear();
    }
  } catch {
    res.status(503).json({ error: "Alert scope is temporarily unavailable. Retry shortly." });
    return;
  }
  try {
  ...
  res.json(ListAlertsResponse.parse(scoped));
    ...
    res.status(503).json({ error: "Alert history is temporarily unavailable. Retry shortly." });
  }
});
