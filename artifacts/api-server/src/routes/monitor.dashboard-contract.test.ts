...
describe("dashboard and spend endpoint contracts", () => {
  test("omitted dashboard choices use full-period reporting and month-end forecasting", () => {
    expect(dashboardRangeType()).toBe("full-term");
    expect(dashboardRangeType("billing")).toBe("billing");
    expect(dashboardRangeType("custom")).toBe("custom");
    expect(projectionTarget({}, "2026-05-20T00:00:00.000Z").kind)
      .toBe("month_end");
  });

  test("resolves fixed term and exact calendar-year projection boundaries", () => {
    expect(projectionTarget(
      { projectionHorizon: "term_end" },
    ...
    const dashboard = GetDashboardResponse.parse({
      ...
      projection: {
        ...
        rate: {
          ...
          windowEndExclusive: "2026-09-03T00:00:00.000Z",
          completeDays: 8,
          sevenDayDailySpendUsd: 9,
          twentyEightDayDailySpendUsd: null,
        },
        baseline: { spendUsd: 18, complete: true },
        budget: null,
        projectedTotalUsd: 261,
        projectedKnownTotalUsd: null,
  ...
  test("invalid custom queries are distinct from unavailable stored accounting", () => {
    ...
    expect(dashboardQueryError({
      projectionHorizon: "planning_end",
    })).toContain("planningEndDate");
    expect(dashboardQueryError({
      projectionHorizon: "month_end",
      planningEndDate: "2026-10-01",
    })).toContain("only valid");
    expect(dashboardQueryError({
      projectionHorizon: "planning_end",
      planningEndDate: "2026-02-30",
    })).toContain("valid planningEndDate");
    expect(dashboardQueryError({
      projectionHorizon: "planning_end",
      planningEndDate: "9999-12-31",
    })).toContain("next five years");
  });
  ...
  test("ordinary dashboard generation is wired only to persisted readers", () => {
    ...
    expect(route).not.toContain("BoundedStaleCache");
    expect(accounting).toContain("BoundedStaleCache");
    expect(accounting).toContain("context.cacheIdentity");
    expect(accounting).not.toContain("canonicalValue(query)");
    ...
    expect(route).toContain("usageForRequest");
    expect(route).toContain("dailyUsageRollups");
    expect(route).not.toMatch(/rawFetch|refreshBillingPeriodMetadata/);
    expect(route).toContain('target.kind !== "cycle_end"');
    expect(route).toContain('target.kind !== "planning_end"');
    expect(route).toContain('target.kind !== "term_end"');
    expect(route).toContain("target.start !== result.period.start");
    expect(route).toContain('observation.status !== "complete"');
