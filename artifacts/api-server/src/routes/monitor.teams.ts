...
export function isTeamCycleAgentUsageComplete(
  usage: Pick<UsageSnapshot, "coverage" | "members">,
  workspaceIds: ReadonlySet<string>,
  groupIds: readonly string[],
  aiSpendByGroup: ReadonlyMap<string, ReadonlyMap<string, number>>,
): boolean {
  if (
    usage.coverage.failedWorkspaceDays.some((day) => workspaceIds.has(day.workspaceId)) ||
    usage.coverage.missingWorkspaceDays.some((day) => workspaceIds.has(day.workspaceId))
  ) return false;
  for (const groupId of groupIds) {
    for (const userId of aiSpendByGroup.get(groupId)?.keys() ?? []) {
      let observed = false;
      for (const workspaceId of workspaceIds) {
        const entry = usage.members.get(workspaceId)?.get(userId);
        if (!entry) continue;
        observed = true;
        if (entry.agentMetricsComplete !== true) return false;
      }
...
export function qualifyTeamAgentMetrics(
  spendUsd: number,
  ...
  return {
    cycleAgentSpendUsd: spendUsd,
    agentRemainingUsd: monthlyLimitUsd === null ? null : monthlyLimitUsd - spendUsd,
    agentPercentUsed: monthlyLimitUsd === null
      ? null
      : (spendUsd / monthlyLimitUsd) * 100,
    agentBlocked: monthlyLimitUsd === null ? null : spendUsd >= monthlyLimitUsd,
  };
}
      ...
      budgets: visibleBudgets.map((b) => {
        ...
        const usageComplete = !billing.isFallback &&
          isTeamCycleAgentUsageComplete(
            ...
            cycleUsage.rollup.aiSpendByGroup,
          );
        const agentMetrics = qualifyTeamAgentMetrics(
          cycleAgentSpendUsd,
...
router.get(
  ...
  async (req, res): Promise<void> => {
    const parsed = GetTeamAllocationAuditQueryParams.safeParse(req.query);
    if (
      !parsed.success ||
      (
        parsed.data.beforeId !== undefined &&
        !Number.isSafeInteger(parsed.data.beforeId)
      )
    ) {
      res.status(400).json({ error: "Invalid allocation audit cursor" });
      return;
    }
    const hiddenTeamNames = req.authz!.isTrueAccountAdmin
      ? new Set<string>()
      : new Set(
          (
            await db.select({ teamName: teamBudgetsTable.teamName })
              .from(teamBudgetsTable)
              .where(eq(teamBudgetsTable.isHidden, true))
          ).map((team) => team.teamName),
        );
    const visibleChanges = await getTeamAllocationAudits({
      beforeId: parsed.data.beforeId,
      hiddenTeamNames: [...hiddenTeamNames],
      fields: req.authz!.isTrueAccountAdmin
        ? undefined
        : ["annualAllocationUsd", "monthlyAllocationAddition"],
    });
...
router.post(
  "/admin/team-budgets/:teamName/allocations",
  requireCapability("canEditAllocations"),
  async (req, res): Promise<void> => {
    const params = AddTeamMonthlyAllocationParams.safeParse(req.params);
    const body = AddTeamMonthlyAllocationBody.safeParse(req.body);
    const bodyKeys = req.body && typeof req.body === "object"
    ...
    if (
      !params.success ||
      !body.success ||
      bodyKeys.join(",") !== "amountUsd,idempotencyKey,month"
    ) {
      res.status(400).json({
        error: !params.success
          ? params.error.message
          : !body.success
            ? body.error.message
            : "Body must contain only month, amountUsd, and idempotencyKey",
      });
      return;
    }

    const result = await addTeamMonthlyAllocation({
      teamName: params.data.teamName,
      month: body.data.month,
      amountUsd: body.data.amountUsd,
      idempotencyKey: body.data.idempotencyKey,
      actorUserId: req.user!.id,
      allowHidden: req.authz!.isTrueAccountAdmin,
    });
    ...
    if (result.status === "conflict") {
      res.status(409).json({
        error: "Idempotency key was already used with a different allocation payload",
      });
      return;
    }

    const snapshot = await getEffectiveTeamBudgets();
    res.status(201).json(AddTeamMonthlyAllocationResponse.parse(
      serializeTeamBudgetHistoryTeam(result.team, snapshot.adjustments),
    ));
  },
);
