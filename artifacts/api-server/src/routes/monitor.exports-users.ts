...
import { buildSnapshotGroupTeamMap } from "./monitor.shared";
...
router.get("/export/users.csv", async (req, res): Promise<void> => {
  ...
  const teamNameMap = buildSnapshotGroupTeamMap(
    visible,
    dir,
  ...
  // Read only workspaces that can contribute to the requested canonical
  // groups. Unrelated authorized workspace gaps must not qualify this export.
  const scopedWorkspaceIds = new Set(
    exportGroups.map((group) => group.workspaceId),
  );
  ...
  const hasValidObservation = snapshot.coverage.presentWorkspaceDays > 0;
  // Do not imply a spend ranking when retained numeric facts have no valid
  // observation for the requested interval.
  rows.sort((a, b) => hasValidObservation
    ? b.spendUsd - a.spendUsd ||
      a.username.localeCompare(b.username, undefined, { sensitivity: "base" })
    : a.username.localeCompare(b.username, undefined, { sensitivity: "base" }));
  const isComplete = canonical.isComplete;
  const dataStatus = snapshot.status === "empty"
    ? "empty"
    : isComplete
      ? snapshot.status
      : "partial";
  const missingCoverage = [
    ...snapshot.coverage.missingWorkspaceDays.map(
      (item) => `${item.workspaceId}:${item.usageDate}`),
    ...snapshot.coverage.missingAccountDays.map((date) => `account:${date}`),
  ];
  const failedCoverage = snapshot.coverage.failedWorkspaceDays.map(
    (item) => `${item.workspaceId}:${item.usageDate}`);
  const qualifications = [
    ...(!hasValidObservation
      ? ["No valid usage observation is available; amount columns are blank."]
      : []),
    ...(hasValidObservation && !isComplete
      ? ["Partial usage coverage; exported amounts include only observed facts and missing facts are not zero."]
      : []),
    ...(snapshot.status === "stale"
      ? ["Usage observations are stale."]
      : []),
    ...(snapshot.latestFailedAttempts.length > 0
      ? ["A newer usage refresh failed; last successful facts may be shown."]
      : []),
    ...(![...scopedWorkspaceIds].every((workspaceId) =>
      projectMetadata.completeWorkspaceIds.has(workspaceId))
      ? ["Project attribution metadata is incomplete for the exported workspaces."]
      : []),
  ];
  const qualificationCells = [
    ...
    snapshot.coverage.requestedDays,
    missingCoverage.join(";"),
    failedCoverage.join(";"),
    qualifications.join(" "),
  ];
  const headerCells = [
    "Email", "Name", "Username", "Internal Replit User", "Workspace(s)",
    "Group", "Team", "Eligible AI Spend (USD)",
    "Eligible Hosting / Non-AI Spend (USD)", "Eligible Spend (USD)",
    "Record Kind", "Range Start", "Range End Exclusive", "Data Status",
    "Data As Of", "Data Stale", "Coverage Ratio", "Coverage Requested Days",
    "Coverage Missing Days", "Coverage Failed Workspace Days",
    "Data Qualifications",
  ];
  const lines = rows.map((row) => [
    ...
    row.group,
    row.team,
    hasValidObservation ? row.aiSpendUsd.toFixed(2) : "",
    hasValidObservation ? row.nonAiSpendUsd.toFixed(2) : "",
    hasValidObservation ? row.spendUsd.toFixed(2) : "",
    "user",
    ...qualificationCells,
  ...
  const csv = [
    headerCells.map(escapeCsvCell).join(","),
    ...lines,
  ].join("\r\n") + "\r\n";
  ...
  res.setHeader("X-Data-As-Of", snapshot.dataAsOf ?? "");
  res.setHeader("X-Coverage-Ratio", String(snapshot.coverage.ratio));
  res.send(csv);
});
...
router.get("/users/activity", async (req, res): Promise<void> => {
  ...
  const [snapshot, projectMetadata] = await Promise.all([
    readUsageSnapshot({
      window: selection.window,
      workspaceIds: scopedWorkspaceIds,
      includeAccountAnchor: callerIsAccountAdmin,
    }),
    readProjectMetadata(scopedWorkspaceIds),
  ]);
  const scopedMembers = visibleGroupMembers(req.authz!, dir.groupMembers);
