...
import {
  durableFailedUnitKeys,
  presentUsageIngestRun,
  type IngestRunRow,
...
async function presentActiveUsageIngestRuns(rows: Array<Record<string, unknown>>) {
  const latestFailure = new Map<string, string>();
  for (const row of rows) {
    if (!row.finished_at) continue;
    const finishedAt = new Date(row.finished_at as string | Date).toISOString();
    for (const key of durableFailedUnitKeys(row.error)) {
      if (!latestFailure.has(key)) latestFailure.set(key, finishedAt);
    }
  }
  const keys = [...latestFailure.keys()];
  if (keys.length === 0) return rows.map(presentUsageIngestRun);
  const activeResult = await pool.query(
    `with failed(key,failed_at) as (
       select * from unnest($1::text[],$2::timestamptz[])
     ),
       parsed as (
         select key,failed_at,split_part(key,'|',1)::date usage_date,
                split_part(key,'|',2) scope_id
         from failed where key ~ '^\\d{4}-\\d{2}-\\d{2}\\|[^|]*$'
       )
     select p.key
     from parsed p
     left join usage_workspace_day w
       on p.scope_id<>'' and w.workspace_id=p.scope_id and w.usage_date=p.usage_date
     left join usage_account_day a
       on p.scope_id='' and a.usage_date=p.usage_date
     where (p.scope_id<>'' and (w.fetched_at is null or p.failed_at>w.fetched_at))
        or (p.scope_id='' and (a.fetched_at is null or p.failed_at>a.fetched_at))`,
    [keys, keys.map((key) => latestFailure.get(key)!)],
  );
  const active = new Set(activeResult.rows.map((row) => String(row.key)));
  return rows.map((row) => {
    const failedKeys = durableFailedUnitKeys(row.error).filter((key) => active.has(key));
    return presentUsageIngestRun({
      ...row,
      error: durableFailedUnitKeys(row.error).length > 0
        ? failedKeys.length > 0 ? `failed-units:${failedKeys.join("\n")}` : null
        : row.error,
    } as IngestRunRow);
  });
}
...
router.get("/status", requireCapability("canManageSystem"), async (_req, res): Promise<void> => {
  ...
  const [emailConfigured, notificationSettings, directoryResult] = await Promise.all([
    isEmailConfigured(),
    getNotificationSettings(),
    getDirectory()
      .then((directory) => ({ directory }))
      .catch((error) => {
        _req.log.warn({ err: error }, "System status directory detail unavailable");
        return { directory: null };
      }),
  ]);
  const fallbackWorkspaceResult = directoryResult.directory
    ? null
    : await pool.query("select distinct workspace_id from usage_workspace_day order by workspace_id");
  const workspaceIds = directoryResult.directory
    ? [...directoryResult.directory.workspaces.keys()]
    : fallbackWorkspaceResult!.rows.map((row) => String(row.workspace_id));
  const projectMetadata = await readProjectMetadata(workspaceIds).catch((error) => {
    _req.log.warn({ err: error }, "System status project metadata detail unavailable");
    return null;
  });
  const completeProjectWorkspaceIds =
    projectMetadata?.completeWorkspaceIds ?? new Set<string>();
  const projectFreshness = [...(projectMetadata?.freshnessByWorkspace.values() ?? [])];
  const successfulProjectWorkspaces = completeProjectWorkspaceIds.size;
  const failedProjectWorkspaceIds = new Set(
    [...(projectMetadata?.freshnessByWorkspace ?? new Map())]
      .filter(([, state]) => state.status === "failed")
      .map(([workspaceId]) => workspaceId),
  );
  const failedProjectWorkspaces = failedProjectWorkspaceIds.size;
  const observedProjectWorkspaces = new Set([
    ...completeProjectWorkspaceIds,
    ...failedProjectWorkspaceIds,
  ]).size;
  const latestProjectSuccess = projectFreshness.reduce<Date | null>(
    (latest, state) =>
      state.lastSuccessfulAt && (!latest || state.lastSuccessfulAt > latest)
  ...
  const [runsResult, backfillResult, reconciliationResult] = await Promise.all([
    pool.query(`select id,kind,started_at,finished_at,units,calls,failures,error,remaining
    ...
    pool.query(`with dates as (
        ...
        select w.workspace_id,d.usage_date
         from unnest($2::text[]) w(workspace_id)
        cross join dates d
      ...
      from workspace_missing,account_missing`, [
  ...
  const recentRuns = await presentActiveUsageIngestRuns(runsResult.rows);
  res.json(GetStatusResponse.parse({
      ...
      projectEnrichment: {
        ...
        successfulWorkspaces: successfulProjectWorkspaces,
        failedWorkspaces: failedProjectWorkspaces,
        pendingWorkspaces: Math.max(
          0,
           workspaceIds.length - observedProjectWorkspaces,
        ),
        lastSuccessfulAt: latestProjectSuccess?.toISOString() ?? null,
      },
      rateLimitTelemetry: {
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const result = await pool.query(
      `select id,kind,started_at,finished_at,units,calls,failures,error,remaining
       from ingest_run order by started_at desc limit $1`,
      [limit],
    );
    res.json(await presentActiveUsageIngestRuns(result.rows));
  },
);
