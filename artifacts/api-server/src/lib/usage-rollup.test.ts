...
test("excludes overlapping internal Agent and creator non-Agent spend exactly once", () => {
  ...
  expect(result.excludedInternalSpendByGroupUser.get("a-group"))
    .toEqual(new Map([["internal", 16]]));
  expect(result.excludedInternalSpendByGroupUser.get("z-group"))
    .toEqual(new Map());
  expect(result.excludedInternalSpendByWorkspace.get("workspace-1")).toBe(16);
  expect(result.excludedInternalAgentSpendByWorkspaceUser)
    .toEqual(new Map([["workspace-1", new Map([["internal", 10]])]]));
  expect(result.excludedInternalOtherSpendByWorkspaceUser)
    .toEqual(new Map([["workspace-1", new Map([["internal", 6]])]]));
  expect(result.excludedInternalAgentSpendByGroupUser?.get("a-group"))
    .toEqual(new Map([["internal", 10]]));
  expect(result.excludedInternalOtherSpendByGroupUser?.get("a-group"))
    .toEqual(new Map([["internal", 6]]));
...
test("retains self-reportable internal usage across workspaces without groups", () => {
  const result = computeSnapshotUsageRollup({
    snapshot: snapshot({
      workspaceIds: ["personal", "shared"],
      members: new Map([
        ["personal", new Map([
          ["self", { totalCostUsd: 6, aiCostUsd: 6 }],
          ["peer", { totalCostUsd: 90, aiCostUsd: 90 }],
        ])],
        ["shared", new Map([
          ["self", { totalCostUsd: 4, aiCostUsd: 4 }],
          ["peer", { totalCostUsd: 80, aiCostUsd: 80 }],
        ])],
      ]),
      workspaces: new Map([
        ["personal", {
          totalCostUsd: 96,
          memberAttributableUsd: 96,
          memberUnattributableUsd: 0,
        }],
        ["shared", {
          totalCostUsd: 84,
          memberAttributableUsd: 84,
          memberUnattributableUsd: 0,
        }],
      ]),
      accountTotalUsd: 180,
    ...
    groups: [],
    membersByGroup: new Map(),
    internalUserIds: new Set(["self", "peer"]),
    projectInfoByWorkspace: new Map(),
  });

  expect(result.excludedInternalAgentSpendByWorkspaceUser).toEqual(new Map([
    ["personal", new Map([["peer", 90], ["self", 6]])],
    ["shared", new Map([["peer", 80], ["self", 4]])],
  ]));
  expect(result.grossSpendUsd).toBe(180);
  expect(result.excludedInternalSpendUsd).toBe(180);
  expect(result.eligibleSpendUsd).toBe(0);
});
...
test("reconciles snapshots with all internal or no internal usage", () => {
  ...
  expect(allInternal.excludedInternalSpendByGroupUser.get("a-group"))
    .toEqual(new Map([["member", 7]]));
