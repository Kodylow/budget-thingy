...
test("repeat and restarted snapshot passes cannot rewrite an immutable day", async () => {
  ...
  assert.equal(repeatedAfterRestart, false);
  assert.deepEqual(store.days.get("2026-08-26")[0].userIds, ["u-1"]);
});
...
test("an incomplete group-members refresh is rejected instead of freezing an empty roster", () => {
  ...
  assert.throws(
    () => assertCompleteRosterDirectory(incomplete),
      /Roster directory refresh incomplete/,
  );
});