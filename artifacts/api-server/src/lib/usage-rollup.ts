...
export interface SnapshotUsageRollup extends DedupedUsageRollup {
  ...
  /** Excluded internal spend by group and internal creator/member identity. */
  excludedInternalSpendByGroupUser: Map<string, Map<string, number>>;
  /** Excluded internal spend by authoritative workspace. */
  excludedInternalSpendByWorkspace: Map<string, number>;
  /** Internal Agent spend by workspace and actor, retained for self reporting. */
  excludedInternalAgentSpendByWorkspaceUser?: Map<string, Map<string, number>>;
  /** Internal creator-attributed non-Agent spend retained for self reporting. */
  excludedInternalOtherSpendByWorkspaceUser?: Map<string, Map<string, number>>;
  /** Internal Agent spend by group and actor, retained for self-only rows. */
  excludedInternalAgentSpendByGroupUser?: Map<string, Map<string, number>>;
  /** Internal non-Agent spend by group and creator, retained for self-only rows. */
  excludedInternalOtherSpendByGroupUser?: Map<string, Map<string, number>>;
  ...
  /** Raw actor Agent spend by workspace, retained for exact self reporting. */
  agentSpendByWorkspaceUser?: Map<string, Map<string, number>>;
  /** Raw creator-attributed non-Agent spend by workspace for self reporting. */
  otherSpendByWorkspaceUser?: Map<string, Map<string, number>>;
...
export function computeSnapshotUsageRollup(
  ...
  const agentSpendByWorkspaceUser = new Map<string, Map<string, number>>();
  const otherSpendByWorkspaceUser = new Map<string, Map<string, number>>();
  ...
  const excludedInternalSpendByGroupUser = new Map(
    ordered.map((group) => [group.id, new Map<string, number>()]),
  );
  const excludedInternalSpendByWorkspace = new Map<string, number>();
  const excludedInternalAgentSpendByWorkspaceUser = new Map<
    string,
    Map<string, number>
  >();
  const excludedInternalOtherSpendByWorkspaceUser = new Map<
    string,
    Map<string, number>
  >();
  const excludedInternalAgentSpendByGroupUser = new Map(
    ordered.map((group) => [group.id, new Map<string, number>()]),
  );
  const excludedInternalOtherSpendByGroupUser = new Map(
    ordered.map((group) => [group.id, new Map<string, number>()]),
  );
  ...
  for (const workspaceId of scopedWorkspaceIds(snapshot)) {
    ...
    for (const [userId, usage] of memberEntries) {
      ...
      const workspaceAgentUsers =
        agentSpendByWorkspaceUser.get(workspaceId) ?? new Map<string, number>();
      add(workspaceAgentUsers, userId, aiSpendUsd);
      agentSpendByWorkspaceUser.set(workspaceId, workspaceAgentUsers);
      if (internalUserIds.has(userId)) {
        excludedInternalSpendUsd += aiSpendUsd;
        const workspaceUsers =
          excludedInternalAgentSpendByWorkspaceUser.get(workspaceId) ??
          new Map<string, number>();
        add(workspaceUsers, userId, aiSpendUsd);
        excludedInternalAgentSpendByWorkspaceUser.set(workspaceId, workspaceUsers);
        const owner = owners.get(userId);
        if (owner) {
          add(excludedInternalSpendByGroup, owner.id, aiSpendUsd);
          add(excludedInternalSpendByGroupUser.get(owner.id)!, userId, aiSpendUsd);
          add(
            excludedInternalAgentSpendByGroupUser.get(owner.id)!,
            userId,
            aiSpendUsd,
          );
        }
    ...
    for (const [projectId, usage] of projectEntries) {
      ...
      if (creatorId !== null) {
        const workspaceOtherUsers =
          otherSpendByWorkspaceUser.get(workspaceId) ?? new Map<string, number>();
        add(workspaceOtherUsers, creatorId, attributedNonAiSpendUsd);
        otherSpendByWorkspaceUser.set(workspaceId, workspaceOtherUsers);
      }
      if (creatorId !== null && internalUserIds.has(creatorId)) {
        excludedInternalSpendUsd += attributedNonAiSpendUsd;
        const workspaceUsers =
          excludedInternalOtherSpendByWorkspaceUser.get(workspaceId) ??
          new Map<string, number>();
        add(workspaceUsers, creatorId, attributedNonAiSpendUsd);
        excludedInternalOtherSpendByWorkspaceUser.set(workspaceId, workspaceUsers);
        if (owner) {
          add(excludedInternalSpendByGroup, owner.id, attributedNonAiSpendUsd);
          add(
            excludedInternalSpendByGroupUser.get(owner.id)!,
            creatorId,
            attributedNonAiSpendUsd,
          );
          add(
            excludedInternalOtherSpendByGroupUser.get(owner.id)!,
            creatorId,
            attributedNonAiSpendUsd,
          );
        }
  ...
  return {
    ...
    excludedInternalSpendByGroupUser,
    excludedInternalSpendByWorkspace,
    excludedInternalAgentSpendByWorkspaceUser,
    excludedInternalOtherSpendByWorkspaceUser,
    excludedInternalAgentSpendByGroupUser,
    excludedInternalOtherSpendByGroupUser,
    eligibleSpendUsd,
    totalMemberCount,
    pendingCount,
    isComplete:
    ...
    aiSpendByUser,
    nonAiSpendByUser,
    agentSpendByWorkspaceUser,
    otherSpendByWorkspaceUser,
    aiSpendByGroup,
    nonAiSpendByGroup,
...
export function usageSnapshotForDay(
  ...
  const missingAccountDays = !snapshot.includesAccountAnchor || hasAccountDay
    ? [] : [usageDate];
  const totalExpected = workspaceIds.length +
    (snapshot.includesAccountAnchor ? 1 : 0);
  const presentWorkspaceDays = [...workspaces.keys()].filter(
    (workspaceId) => !failedWorkspaceIds.has(workspaceId),
  ).length;
  const totalPresent = presentWorkspaceDays +
    (snapshot.includesAccountAnchor && hasAccountDay ? 1 : 0);
  const isPartial =
  ...
  return {
    ...
    status: isPartial
      ...
      : "complete",
    workspaceStatus:
      failedWorkspaceDays.length > 0 || missingWorkspaceDays.length > 0
        ? "partial"
        : snapshot.workspaceStatus === "stale"
        ? "stale"
        : "complete",
    coverage: {
      requestedDays: 1,
