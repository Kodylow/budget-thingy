...
vi.mock("./enterprise", async () => {
  ...
  return {
    ...
    getBillingPeriodMetadata: () => ({
      ...
      isFresh: true,
      isFallback: false,
      differsFromReportingCutoff: true,
      label: "July 2099",
    }),
  };
});
...
beforeEach(async () => {
  ...
  await pglite.exec(`
    ...
    CREATE TABLE ingest_run (
      id SERIAL PRIMARY KEY, kind TEXT NOT NULL, started_at TIMESTAMPTZ NOT NULL,
      finished_at TIMESTAMPTZ, units INTEGER NOT NULL DEFAULT 0,
      calls INTEGER NOT NULL DEFAULT 0, failures INTEGER NOT NULL DEFAULT 0,
      error TEXT, remaining INTEGER
    );
    ...
    CREATE TABLE api_project_metadata_state (
      workspace_id TEXT PRIMARY KEY, status TEXT NOT NULL,
      deployment_status_observed BOOLEAN NOT NULL DEFAULT false,
      error_message TEXT, started_at TIMESTAMPTZ NOT NULL,
      completed_at TIMESTAMPTZ NOT NULL, last_successful_at TIMESTAMPTZ
    );
...
async function insertCompleteUsage(
  ...
  for (const workspaceId of new Set(["ws-1", "ws-2", ...Object.keys(byWorkspace)])) {
    ...
    await pglite.query(
      `INSERT INTO api_project_metadata_state
         (workspace_id,status,started_at,completed_at,last_successful_at)
       VALUES ($1,'success',$2,$2,$2)`,
      [workspaceId, NOW],
    );
  }
  for (let day = 1; day <= 15; day++) {
