...
describe("isEmailConfigured", () => {
  ...
  it("routes non-production delivery only to the configured operator and marks the subject", async () => {
  ...
  it("fixes test delivery to the configured operator in production", async () => {
    process.env.NODE_ENV = "production";
  ...
  it("fails closed without a configured safe recipient", async () => {
    delete process.env.BOOTSTRAP_ADMIN_EMAIL;
    setSendEmailOverrideForTests(vi.fn());

    await expect(
      sendEmail(["intended@example.com"], "Alert", "<p>Alert</p>"),
    ).resolves.toEqual({
      ok: false,
      error: "BOOTSTRAP_ADMIN_EMAIL is required for safe email delivery",
      deliveredTo: [],
    });
    expect(connectorMocks.constructor).not.toHaveBeenCalled();
  });
...
describe("buildAlertEmail", () => {
  ...
  it("integrates server-owned test delivery context into the branded email", () => {
    ...
    expect(rendered.html).toContain("safe preview sent only to the configured operator");
