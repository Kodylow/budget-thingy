...
test("member additions and removals affect only roster dates after the change", () => {
  ...
  assert.equal(before.spendByGroup.get(alpha.id), 12);
  assert.equal(afterRemoval.spendByGroup.get(alpha.id), 0);
  assert.equal(afterAddition.spendByGroup.get(alpha.id), 8);
  assert.equal(before.totalSpendUsd, 23, "ungrouped users and no-user spend remain in totals");
});
...
test("deleted directory members remain attributable by stable roster user ID", () => {
  ...
  assert.equal(result.spendByGroup.get(alpha.id), 8);
});
...
test("overlapping memberships are deduped by stable group ordering", () => {
  ...
  assert.equal(result.spendByGroup.get(alpha.id), 12);
  assert.equal(result.spendByGroup.get(beta.id), 0);
  assert.equal(result.totalSpendUsd, 23);
});
...
test("same-name migration aliases merge their historical source spend", () => {
  ...
  assert.equal(merged.get("g-primary"), 12);
});
