...
describe("annual allocation seed", () => {
  it("seeds canonical DXP and Non-DXP budgets idempotently", async () => {
    ...
    try {
      const firstResult = await applyAnnualTeamBudgetBackfill();
      const secondResult = await applyAnnualTeamBudgetBackfill();
      ...
      expect(firstResult.legacyBudgetRequiresReview).toBe(true);
      expect(secondResult.legacyBudgetRequiresReview).toBe(true);
      expect(splitBudgets).toHaveLength(3);
