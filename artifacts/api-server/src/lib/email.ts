...
import { getBootstrapAccountAdminEmail, normalizeEmail } from "./authz";
...
export async function sendEmail(
  ...
  const safeRecipient = getBootstrapAccountAdminEmail();
  const deliveredTo =
...
export function getEmailTestRecipient(): string {
  return getBootstrapAccountAdminEmail() ?? "";
}
                  ...
                  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%; border-collapse:separate; background:#eafafd; border:1px solid #9ddfea; border-radius:7px;">
                    <tr>
                      <td style="padding:12px 14px; color:#155f6c; font-size:13px; line-height:19px;">
                        ...
                        <span>This is a safe preview sent only to the configured operator. It does not change alert activity or threshold state.</span>
                      </td>
                    </tr>
                  </table>
                </td>
