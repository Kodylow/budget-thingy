        ...
        <div className="flex gap-2 items-start text-sm text-blue-600 dark:text-blue-400">
          ...
          <div className="flex-1 min-w-0">
            ...
            <ul className="list-disc pl-4 space-y-1 opacity-90 max-h-32 overflow-y-auto">
              {overrides.map(o => (
                <li key={o.userId} className="break-all">User <code>{o.userId}</code> kept manual limit of ${o.previousAmountUsd?.toFixed(2) ?? '0.00'}</li>
              ))}
            </ul>
          </div>
        </div>
      ...
      {failures.length > 0 && (
        <div className="flex gap-2 items-start text-sm text-destructive">
          ...
          <div className="flex-1 min-w-0">
            ...
            <ul className="list-disc pl-4 space-y-1 opacity-90 max-h-32 overflow-y-auto">
              {failures.map(o => (
                <li key={o.userId} className="break-words">User <code className="break-all">{o.userId}</code>: {o.error || 'Unknown error'}</li>
              ))}
            </ul>
          </div>
        </div>
  ...
  return (
    <div className="mb-4">
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 rounded-md border bg-muted/20 p-3">
        ...
        />
        <Button className="w-full sm:w-auto" onClick={handleApply} disabled={setPolicy.isPending || (value === (currentAmount?.toString() || ''))}>
          {setPolicy.isPending ? 'Applying...' : 'Apply Baseline'}
        </Button>
        {currentAmount !== null && (
          <Button variant="ghost" className="w-full justify-center px-2 text-destructive sm:w-auto" onClick={() => { setValue(''); setDialogOpen(true); }} disabled={setPolicy.isPending}>
