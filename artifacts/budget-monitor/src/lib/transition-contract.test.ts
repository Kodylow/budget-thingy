...
describe('protected query transition contract', () => {
  ...
  it('clears before preview publication and remounts component state at auth boundaries', async () => {
    ...
    expect(authSource).toMatch(
      /void queryClient\.cancelQueries\(\);\s+queryClient\.clear\(\);\s+clearApiDiagnostics\(\);\s+setPreviewState\(next\)/,
    );
    ...
    expect(authSource).toContain("from './auth-context-definition'");
    expect(authSource).not.toContain('createContext(');
    expect(appSource).toContain('<Router key={authorizationKey} />');
    // Resource 403s only revalidate; confirmed access changes/failures above
    // remain the destructive boundary. Runtime batch behavior is tested separately.
    const bridge = appSource.slice(
      appSource.indexOf('function AuthorizationFailureBridge()'),
      appSource.indexOf('function RouteLoading()'),
    );
    expect(bridge).toContain("availability === 'authorized'");
    expect(bridge).toContain('createForbiddenRevalidator(revalidateAuthorization)');
    expect(bridge).not.toContain('queryClient.clear()');
    expect(bridge).not.toContain('retryAuthorization()');
    expect(appSource.indexOf('<AuthorizationFailureBridge />')).toBeLessThan(
      appSource.indexOf('<AuthGate>'),
    );
    expect(authSource).toContain('revalidateAuthorization');
