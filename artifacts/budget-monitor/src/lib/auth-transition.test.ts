...
import { createForbiddenRevalidator, protectedAuthorizationFingerprint } from './auth-transition';
...
describe('protected authorization fingerprint', () => {
  ...
  it.each([
    ['authorizationRevision', { ...auth, authorizationRevision: 'revision-2' }],
...
describe('resource denial revalidation batches', () => {
  it('coalesces slow checks, retains ordinary denials, and checks later revocations again', async () => {
    let now = 1_000;
    let finish!: () => void;
    const revalidate = vi.fn(() => new Promise<void>((resolve) => { finish = resolve; }));
    const onForbidden = createForbiddenRevalidator(revalidate, () => now);
    onForbidden();
    onForbidden();
    now += 20_000;
    onForbidden();
  ...
  it('does not permanently latch after a failed check', async () => {
    let now = 1_000;
    const revalidate = vi.fn().mockRejectedValue(new Error('unavailable'));
    const onForbidden = createForbiddenRevalidator(revalidate, () => now);
    onForbidden();
    await Promise.resolve();
    onForbidden();
