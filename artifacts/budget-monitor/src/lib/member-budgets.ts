...
export type MemberBudgetDisplayState = 'loading' | 'unavailable' | 'unknown' | 'known';

export function getMemberBudgetDisplayState(input: {
  hasResponse: boolean;
  isLoading: boolean;
  isError: boolean;
  observationStatus?: 'complete' | 'failed' | 'unavailable' | 'refreshing';
  lastSuccessfulAt?: number | null;
  hasMemberBudget: boolean;
}): MemberBudgetDisplayState {
  if (!input.hasResponse) {
    if (input.isLoading) return 'loading';
    return input.isError ? 'unavailable' : 'unknown';
  }
  if (!input.hasMemberBudget) return 'unknown';
  const hasAuthoritativeObservation =
    input.observationStatus === 'complete' || input.lastSuccessfulAt != null;
  return hasAuthoritativeObservation ? 'known' : 'unknown';
}
