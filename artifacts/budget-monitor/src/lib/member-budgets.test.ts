...
test('missing budgets are known-empty only after an authoritative observation', () => {
  expect(getMemberBudgetDisplayState({
    hasResponse: false,
    isLoading: false,
    isError: true,
    hasMemberBudget: false,
  })).toBe('unavailable');
  expect(getMemberBudgetDisplayState({
    hasResponse: true,
    isLoading: false,
