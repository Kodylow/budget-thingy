...
import { invalidateBudgetCaches } from './budget-cache';
...
test('committed budget changes refresh compact details without reviving old queries', () => {
  const client = new QueryClient();
  const detailKey = ['/api/reporting/details/group-a,group-b', { rangeType: 'mtd' }];
  const unrelatedKey = ['/api/email-settings'];
  client.setQueryData(detailKey, { members: [] });
  client.setQueryData(unrelatedKey, {});
  invalidateBudgetCaches(client, 'workspace-a');
