...
describe("authorization request state", () => {
  it("fails closed when an authorized response lacks a complete revision", async () => {
    const { authorizationRevision, ...auth } = authorizedEnvelope.auth;
    await expect(loadAuthorization({
      previewAs: null,
      signal: new AbortController().signal,
      fetcher: vi.fn(async () => Response.json({ ...authorizedEnvelope, auth })),
