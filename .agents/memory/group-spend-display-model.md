...
**How to apply:**
Use canonical group/team maps for every row, rollup, budget, trend, alert, and member-detail calculation; defer rather than calculate from incomplete authoritative usage. Do not block a total-only display on optional decomposition feeds. For account-wide headline/footer totals, use the gross anchor and render gross minus canonical as the residual row. Expose neither field to scoped viewers. Label stored alert values as send-time snapshots and project tables as attribution.
