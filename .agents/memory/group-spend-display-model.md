...
**How to apply:**
Use canonical group/team maps for every row, rollup, budget, trend, alert, and member-detail calculation. When a canonical response contains a numeric value, show it without provisional styling; use a loader only before the first value exists. Surface partial, stale, or failed refreshes through a deduplicated transient notification rather than persistent page chrome. Do not block a total-only display on optional decomposition feeds. For account-wide headline/footer totals, use the gross anchor and render gross minus canonical as the residual row. Expose neither field to scoped viewers. Label stored alert values as send-time snapshots and project tables as attribution.
