...
- `/usage` can return `hasMore: true` with no cursor. **Why:** treating it as an ordinary exception leaves scopes pending forever. **How to apply:** time-shard serially; persist unreachable minimal shards as partial.
